...
def test_post_story_response() -> None:
    mock_response = {
        "metadata": {
    ...
    assert item.story_id == mock_response["data"]["storyId"]
...
def test_get_story_response() -> None:
    mock_response = {
        "metadata": {
        ...
        "data": {
            ...
            "storyResult": {
                "title": "TEst story",
                "textSettings": {
                    "useVoiceIntelligenceLayer": False
                },
                "voices": [
                    {
    ...
    mock_response_id = mock_response["data"]["audioforms"][0]["header"]["audioformId"]

    item = Story.Item(mock_response)
    assert item.story_id == mock_response["data"]["storyId"]
    assert item.audioform_status_code == 200
    assert item.audioform_ids == [mock_response_id]
    assert item.audioforms == {mock_response_id: mock_response["data"]["audioforms"][0]}
    assert item.results == {mock_response_id: mock_response["data"]["audioforms"][0]["delivery"]}
