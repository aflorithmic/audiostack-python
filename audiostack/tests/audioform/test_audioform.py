...
def test_audioform_create_v1(mock_send_request: Mock) -> None:
    ...
    mock_response = {
        "data": {
            "audioform_id": "new-audioform-v1-123",
            "status_code": 202
        },
        "statusCode": 202,
        "message": "Audioform created"
    }
    ...
    result = Audioform.create(audioform_config, version="1")

    assert isinstance(result, Audioform.Item)
    assert result.audioform_id == "new-audioform-v1-123"
    assert result.status_code == 202

    expected_audioform = audioform_config.copy()
    expected_audioform["header"] = {"version": "1"}
    mock_send_request.assert_called_once_with(
        rtype=RequestTypes.POST,
        route="",
        json={"audioform": expected_audioform}
    )
...
def test_audioform_create_v001(mock_send_request: Mock) -> None:
    ...
    mock_response = {
        "data": {
            "audioform_id": "new-audioform-v001-123",
            "status_code": 202
        },
        "statusCode": 202,
        "message": "Audioform created"
    ...
    # Realistic v0.0.1 audioform payload with legacy field names
    audioform_config = {
        "assets": {
            "media 0": {
                "type": "media",
    ...
    result = Audioform.create(audioform_config, version="0.0.1")
    ...
    expected_audioform = audioform_config.copy()
    expected_audioform["header"] = {"version": "0.0.1"}
    mock_send_request.assert_called_once_with(
        rtype=RequestTypes.POST,
        route="",
        json={"audioform": expected_audioform}
...
@patch('audiostack.audioform.audioform.Audioform.interface.send_request')
def test_audioform_create_missing_required_fields(mock_send_request: Mock) -> None:
    """Test Audioform.create method with missing required fields"""
    mock_response = {
        "statusCode": 400,
        "message": "Invalid audioform configuration",
        "errors": [
            "assets is required",
    ...
    assert result.status_code == 200  # Default value when no data
...
def test_audioform_create_invalid_version(mock_send_request: Mock) -> None:
    ...
    result = Audioform.create(audioform_config, version="invalid")

    assert isinstance(result, Audioform.Item)
    assert result.audioform_id == ""

    expected_audioform = audioform_config.copy()
    expected_audioform["header"] = {"version": "invalid"}
    mock_send_request.assert_called_once_with(
        rtype=RequestTypes.POST,
        route="",
        json={"audioform": expected_audioform}
...
def test_audioform_get_v1(mock_send_request: Mock) -> None:
    ...
    mock_response = {
        "data": {
            "audioform_id": "existing-audioform-v1-123",
            "status_code": 200,
            "audioform": {
                "header": {"version": "1"},
...
def test_audioform_get_v001(mock_send_request: Mock) -> None:
    ...
    mock_response = {
        "data": {
            "audioform_id": "existing-audioform-v001-123",
            "status_code": 200,
            "audioform": {
            ...
            "result": {
                ...
                "assets": {
                    "script 0": {"type": "tts", "durationInSecond": 4.95}
                },
                "production": {"masteringPreset": "balanced"},
                "delivery": {"encoderPreset": "mp3"}
            }
...
def test_audioform_item_initialisation_partial_data() -> None:
    ...
    response_data = {
        "data": {
            "audioform_id": "partial-test-123",
            # Missing status_code, audioform, result
        },
        "statusCode": 200,
        "message": "Success"
    }
